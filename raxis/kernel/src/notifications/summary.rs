...
fn json_num(v: &Value, key: &str) -> String {
    v.get(key)
        .and_then(|x| x.as_u64().map(|n| n.to_string()).or_else(|| x.as_i64().map(|n| n.to_string())))
        .unwrap_or_else(|| "<?>".to_owned())
}
...
mod tests {
    ...
    fn escalation_submitted_renders_compact_summary() {
        let e = make_event("EscalationSubmitted", serde_json::json!({
            "escalation_id": "esc-1",
            "task_id":       "task-A",
    ...
    fn newlines_in_operator_supplied_reason_are_sanitised() {
        ...
        assert!(!s.contains('\r'));
        assert!(!s.contains('\t'));
        assert!(s.contains("line1 line2 line3 line4"),
