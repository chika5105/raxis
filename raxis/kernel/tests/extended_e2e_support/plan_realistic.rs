...
2. Run the Rust lint stack and capture stdout + stderr + the
   ...
   status=0
   {
     "$cargo_bin" fmt --all -- --check 2>&1 || status=$?
     "$cargo_bin" clippy --all-targets -- -D warnings 2>&1 || status=$?
     echo "raxis_check_sh_exit_code=$status"
   } > out/lint/check-rust.txt
   ```
...
mod tests {
    ...
    fn realistic_toml_decodes_and_carries_executors() {
        ...
        let xfile_allowlist: Vec<&str> = xfile
            ...
            .expect("xfile-refactor path_allowlist present")
            .iter()
