...
impl ImageResolver for ProductionResolver {
    async fn resolve(
        ...
        let registry = registry_hint
            ...
            .ok_or(ImageResolverError::NoRegistryHint { digest: *oci_digest })?;
        ...
        if let Err(e) = verify_rootfs_digest(
            &self.layout.rootfs_image_path(oci_digest),
            oci_digest,
        ).await {
    ...
    fn prune_unreferenced(
        ...
        prune_under(self.layout.root().join("images/sha256"), live_digests)
            .and_then(|images_freed| {
                Ok(images_freed +
...
async fn verify_rootfs_digest(
    ...
    loop {
        let n = file.read(&mut buf).await.map_err(|source| ImageResolverError::Io {
            path: path.to_path_buf(), source,
        })?;
        if n == 0 { break; }
        hasher.update(&buf[..n]);
    ...
    if &actual != expected {
        return Err(ImageResolverError::CacheCorrupted {
            ...
            detail: format!(
                "post-extract verify: expected {expected}, got {actual}",
            ),
        });
    }
    Ok(())
...
fn prune_under(
    ...
    for shard in fs::read_dir(&images_root).map_err(|source| ImageResolverError::Io {
        ...
        for digest_entry in fs::read_dir(shard.path()).map_err(|source| ImageResolverError::Io {
            ...
            let Ok(digest) = format!("sha256:{name}").parse::<OciDigest>() else { continue };
            if live_digests.contains(&digest) { continue; }
            total += dir_size(&digest_dir.path())?;
            fs::remove_dir_all(&digest_dir.path()).map_err(|source| ImageResolverError::Io {
                path: digest_dir.path(), source,
            })?;
        }
    }
    Ok(total)
...
fn prune_blobs_under(
    ...
    for shard in fs::read_dir(&blobs_root).map_err(|source| ImageResolverError::Io {
        ...
        for blob in fs::read_dir(shard.path()).map_err(|source| ImageResolverError::Io {
            ...
            let Some(name) = blob.file_name().to_str().map(str::to_owned) else { continue };
            ...
            let hex = name
                ...
                .unwrap_or(&name);
            let Ok(digest) = format!("sha256:{hex}").parse::<OciDigest>() else { continue };
            if live_digests.contains(&digest) { continue; }
            let meta = blob.metadata().map_err(|source| ImageResolverError::Io {
                path: blob.path(), source,
            })?;
            if meta.is_file() {
                total += meta.len();
...
mod tests {
    ...
    struct TestResolver { inner: ProductionResolver, base_url: String }
    ...
    impl TestResolver {
        async fn resolve_via_http(
            ...
            let resp = self.inner.client.get(&self.base_url).send().await
                .map_err(|e| ImageResolverError::RegistryUnreachable {
                    host: self.base_url.clone(), detail: e.to_string(),
                })?;
            let status = resp.status();
            if status.is_client_error() {
                return Err(match status.as_u16() {
                    401 | 403 => ImageResolverError::RegistryAuthRejected {
                        host: self.base_url.clone(), repository: "test".into(),
                    },
                    404 => ImageResolverError::RegistryNotFound {
                        host: self.base_url.clone(), repository: "test".into(), digest: *digest,
                    },
                    _ => ImageResolverError::RegistryServerError {
                        host: self.base_url.clone(), status: status.as_u16(),
                    },
                });
            }
            ...
            if &actual != digest {
                ...
                return Err(ImageResolverError::DigestMismatch {
                    expected: *digest, actual, path: staging,
                });
            }
    ...
    async fn resolve_pulls_and_stages_a_correctly_hashed_blob() {
        ...
        let (addr, counter) = spawn_oci_fixture(vec![
            (StatusCode::OK, body.to_vec()),
        ]).await;

        let tmp = TempDir::new().unwrap();
        let inner = ProductionResolver::new(
            tmp.path(),
        ...
        let test = TestResolver {
            inner,
            base_url: format!("http://{addr}/v2/test/blobs/sha256:{}", hex::encode(digest.as_bytes())),
        };
    ...
    async fn resolve_returns_digest_mismatch_when_body_disagrees() {
        ...
        let (addr, _) = spawn_oci_fixture(vec![
            (StatusCode::OK, actual_body.to_vec()),
        ]).await;
        ...
        let test = TestResolver {
            inner,
            base_url: format!("http://{addr}/v2/test/blobs/sha256:{}", hex::encode(claimed.as_bytes())),
        };

        let err = test.resolve_via_http(&claimed).await.unwrap_err();
        match err {
            ImageResolverError::DigestMismatch { expected, actual, .. } => {
    ...
    async fn resolve_returns_auth_rejected_on_401() {
        let digest: OciDigest =
            "sha256:22112233445566778899aabbccddeeff00112233445566778899aabbccddeeff"
                .parse().unwrap();
        let (addr, _) = spawn_oci_fixture(vec![
            (StatusCode::UNAUTHORIZED, b"go away".to_vec()),
        ]).await;
        let tmp = TempDir::new().unwrap();
        let inner = ProductionResolver::new(
            tmp.path(),
        ...
        let err = test.resolve_via_http(&digest).await.unwrap_err();
        assert!(matches!(err, ImageResolverError::RegistryAuthRejected { .. }));
    }
    ...
    async fn resolve_returns_server_error_on_503() {
        let digest: OciDigest =
            "sha256:33112233445566778899aabbccddeeff00112233445566778899aabbccddeeff"
                .parse().unwrap();
        let (addr, _) = spawn_oci_fixture(vec![
            (StatusCode::SERVICE_UNAVAILABLE, b"down".to_vec()),
        ]).await;
        let tmp = TempDir::new().unwrap();
        let inner = ProductionResolver::new(
            tmp.path(),
    ...
    async fn cached_resolve_is_a_no_network_op() {
        ...
        assert_eq!(resolved.verified_digest, digest);
        assert_eq!(resolved.rootfs_image_path, resolver.layout().rootfs_image_path(&digest));
    }
