...
impl HttpFetch for KernelMediatedHttpFetch {
    async fn fetch<'a>(
        ...
        let inbound = self
            ...
            .request(&outbound)
            .await
            .map_err(transport_to_fetch_error)?;
...
fn transport_to_fetch_error(e: TransportError) -> HttpFetchError {
    match e {
        TransportError::Frame(fe) => HttpFetchError::Transport(format!("kernel transport: {fe}")),
        ...
        TransportError::NotConfigured => {
            HttpFetchError::Transport("kernel transport: not configured".to_owned())
        }
...
mod tests {
    ...
    async fn kernel_mediated_round_trip_preserves_url_and_body() {
        ...
        assert_eq!(
            stub.last_url.lock().await.as_deref(),
