...
/// Production OpenAI Chat Completions client. The gateway's tproxy
/// redirect is transparent — this struct does not need to know
/// whether it's talking to OpenAI directly or through the gateway.
...
impl ModelClient for OpenAiClient {
    async fn create_message(
        ...
        let raw: OpenAiResponse = serde_json::from_slice(&bytes)
            .map_err(|e| ModelError::Json(e.to_string()))?;
        parse_response(&raw)
    }
}
