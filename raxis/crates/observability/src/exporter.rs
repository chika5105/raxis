...
use crate::protocol::{hex_span_id, hex_trace_id, Frame, Stream, SCHEMA_VERSION};
...
impl RingFileExporter {
    ...
    pub fn open(
        ...
        let spans = SegmentWriter::open(root, Stream::Spans, cfg).map_err(into_io)?;
        let metrics = SegmentWriter::open(root, Stream::Metrics, cfg).map_err(into_io)?;
        Ok(Self {
    ...
    pub fn spans_dir(&self) -> PathBuf {
        self.spans.lock().dir().to_owned()
    }

    /// Path to the metrics subdir; useful for tests.
    pub fn metrics_dir(&self) -> PathBuf {
        self.metrics.lock().dir().to_owned()
    }
...
mod tests {
    ...
    use crate::types::{
        AttrMap, AttrValue, DataPoint, MetricData, MetricName, MetricType, SpanData, SpanKind,
