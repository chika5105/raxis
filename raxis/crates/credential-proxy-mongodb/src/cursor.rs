    ...
    for el in &outer.elements {
        if el.name == "cursor" && el.type_byte == 0x03 {
            ...
            for arr_el in &arr.elements {
                if arr_el.type_byte != 0x03 { continue; }
...
fn rewrite_truncated(outer: &[u8], loc: &CursorBatchLocation, keep: usize) -> Vec<u8> {
    ...
    for (i, (es, ee)) in loc.elem_offsets.iter().take(keep).enumerate() {
        ...
        if elem_bytes.is_empty() { continue; }
        let t = elem_bytes[0];
        let nul = elem_bytes[1..].iter().position(|&b| b == 0).unwrap_or(0);
        if 1 + nul + 1 > elem_bytes.len() { continue; }
...
fn parse_doc(doc: &[u8]) -> Option<ParsedDoc> {
    ...
    if doc[total - 1] != 0x00 { return None; }
    let mut p = 4;
    ...
    while p < total - 1 {
        ...
        if value_end > total - 1 { return None; }
...
pub fn rebuild_op_msg_frame(original_frame: &[u8], new_bson_doc: &[u8]) -> Option<Vec<u8>> {
    if original_frame.len() < wire::HEADER_LEN { return None; }
    let header_bytes: [u8; wire::HEADER_LEN] = original_frame[..wire::HEADER_LEN]
        .try_into().ok()?;
...
pub fn extract_reply_doc(frame: &[u8]) -> Option<&[u8]> {
    if frame.len() < wire::HEADER_LEN + 4 + 1 { return None; }
    let header_bytes: [u8; wire::HEADER_LEN] = frame[..wire::HEADER_LEN]
        .try_into().ok()?;
    let header = wire::MsgHeader::parse(header_bytes);
    if header.op_code != wire::OP_MSG { return None; }
    let total = header.message_length as usize;
    if total > frame.len() { return None; }
    ...
    while i < body.len() {
        ...
        if kind == 0 {
            ...
            let doc_total = i32::from_le_bytes(
                body[i..i + 4].try_into().ok()?,
            ) as usize;
            if i + doc_total > body.len() { return None; }
...
mod tests {
    ...
    fn cap_truncates_batch_and_zeros_id() {
        ...
        let cursor_el = parsed.elements.iter().find(|e| e.name == "cursor")
        ...
        let id_el = cursor_parsed.elements.iter().find(|e| e.name == "id")
            .expect("id present");
        ...
        let batch_el = cursor_parsed.elements.iter()
            .find(|e| e.name == "firstBatch").expect("firstBatch present");
