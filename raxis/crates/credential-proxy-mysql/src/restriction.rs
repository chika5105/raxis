...
//! `allow_only_select` flag is the only V2 MVP knob. We do NOT
//! depend on `raxis-credential-proxy-postgres` to keep proxy crates
//! independent (they each own their wire protocol; they share no
//! cross-protocol code path).
//!
//! Reference: `specs/v2/credential-proxy.md §4.2` — MySQL maps
//! `allow_only_select` to ANSI SQL classification on the COM_QUERY
//! statement text.
...
pub struct Restrictions {
    ...
    /// carrying SQLSTATE `42501` (insufficient privilege) and
    /// recorded as a blocked event.
    #[serde(default)]
...
/// (`/* ... */`), and MySQL's `#`-style line comments before
