...
use raxis_credential_proxy_http::{
    restriction::Restrictions, AuditChannel, AuditEvent, AuthMode, HttpProxy, NoopAuditChannel,
...
async fn drive_request(proxy_addr: SocketAddr, request_bytes: &[u8]) -> (Vec<u8>, Vec<u8>) {
...
async fn bearer_injected_and_host_rewritten() {
    ...
    let proxy = HttpProxy::bind(backend.clone(), cfg, std::sync::Arc::new(NoopAuditChannel))
        .await
        .unwrap();
    ...
    assert!(
        http_status_line(&head).starts_with("HTTP/1.1 200"),
        "expected 200; got {:?}",
        http_status_line(&head)
    );
    ...
    assert_eq!(
        ...
        "auth header was not injected; saw {:?}",
        cap.headers
    );
    assert_eq!(
        cap.headers.get("host").map(String::as_str),
        Some(format!("{}", upstream_addr).as_str()),
        "host header was not rewritten; saw {:?}",
        cap.headers.get("host")
    );
    assert_eq!(
        cap.headers.get("user-agent").map(String::as_str),
        Some("raxis-test/1.0"),
        "agent header was not forwarded; saw {:?}",
        cap.headers.get("user-agent")
    );
...
async fn method_allowlist_blocks_post() {
    ...
    let proxy = HttpProxy::bind(backend, cfg, std::sync::Arc::new(NoopAuditChannel))
        .await
        .unwrap();
    let proxy_addr = proxy.local_addr().unwrap();
    tokio::spawn(proxy.serve());
    ...
    assert!(
        http_status_line(&head).starts_with("HTTP/1.1 405"),
        "expected 405; got {:?}",
        http_status_line(&head)
    );
...
async fn path_prefix_blocks_unscoped() {
    ...
    let _ = captured_rx
        .recv()
        .await
        .expect("upstream should have seen in-prefix request");
...
async fn websocket_upgrade_rejected() {
    ...
    let (head, _body) = drive_request(proxy_addr, req).await;
    assert!(
        http_status_line(&head).starts_with("HTTP/1.1 400"),
        "got {:?}",
        http_status_line(&head)
    );
    assert!(
...
async fn missing_credential_returns_502() {
    ...
    let (head, _body) = drive_request(proxy_addr, req).await;
    assert!(
        http_status_line(&head).starts_with("HTTP/1.1 502"),
        "got {:?}",
        http_status_line(&head)
    );
    assert!(
...
async fn post_body_forwarded_to_upstream() {
    ...
    assert!(
        http_status_line(&head).starts_with("HTTP/1.1 201"),
        "got {:?}",
        http_status_line(&head)
    );
