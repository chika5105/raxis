...
fn run_git(
    ...
    let output = cmd
        .output()
        .map_err(|e| DomainError::Transient(format!("git subprocess failed to spawn: {e}")))?;
...
pub fn is_ancestor(base: &str, head: &str, worktree: &Path) -> Result<bool, DomainError> {
...
pub fn topology_check(base: &str, head: &str, worktree: &Path) -> Result<(), DomainError> {
    let b = validate_sha(base)?;
    let h = validate_sha(head)?;
...
mod tests {
    ...
    fn post_process_strips_dot_slash() {
        assert_eq!(
            post_process_path("./foo.rs").unwrap(),
            PathBuf::from("foo.rs")
        );
    }
...
mod live {
    ...
    fn live_compute_touched_returns_create_and_modify() {
        ...
        let a = r
            .resources
            .iter()
            .find(|r| r.uri == "path:///a.txt")
            .unwrap();
        let b = r
            .resources
            .iter()
            .find(|r| r.uri == "path:///b.txt")
            .unwrap();
