...
//
// Private keys are NEVER sent to the kernel. Signing happens locally
// in the CLI process; only the resulting Ed25519 signature or signed
// IPC payload is sent.
...
///   - A RAXIS genesis authority key PEM ("BEGIN ED25519 PRIVATE KEY")
...
pub fn load_operator_key(path: &Path) -> Result<ed25519_dalek::SigningKey, CliError> {
    ...
    // RAXIS authority/quality keypair format, generated by genesis:
    //
    //   -----BEGIN ED25519 PRIVATE KEY-----
    //   <64 hex chars seed>
    //   -----END ED25519 PRIVATE KEY-----
    //   -----BEGIN ED25519 PUBLIC KEY-----
    //   <64 hex chars public key>
    //   -----END ED25519 PUBLIC KEY-----
    //
    // Keep this parser here because `raxis policy sign policy.toml`
    // signs with the local authority key, while operator IPC requests
    // use normal PKCS#8 operator keys through the same CLI plumbing.
    if pem.lines().any(|l| l.contains("BEGIN ED25519 PRIVATE KEY")) {
        let seed_hex = pem
            .lines()
            .skip_while(|l| !l.contains("BEGIN ED25519 PRIVATE KEY"))
            .nth(1)
            .ok_or_else(|| CliError::Key("malformed ED25519 private key PEM".to_owned()))?
            .trim();
...
pub fn sign_bytes(key: &ed25519_dalek::SigningKey, message: &[u8]) -> String {
    hex::encode(sign_bytes_raw(key, message))
}

/// Sign `message` and return the raw 64-byte Ed25519 signature.
pub fn sign_bytes_raw(key: &ed25519_dalek::SigningKey, message: &[u8]) -> [u8; 64] {
