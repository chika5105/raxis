...
pub fn run_submit(flags: &GlobalFlags, args: &[String]) -> Result<(), CliError> {
    ...
    let req = OperatorRequest::CreateInitiative {
        plan_toml,
        plan_sig_hex,
        submitted_by,
    };
    ...
    handle_response(resp, |ok| {
        // The kernel emits OperatorResponse::InitiativeCreated whose
        // payload carries `initiative_id` and `status`. We display both
        // — the CLI argument `initiative_id` is what the user typed,
        // not necessarily the kernel-assigned UUID.
