...
pub fn run(flags: &GlobalFlags, args: &[String]) -> Result<(), CliError> {
    ...
    while i < args.len() {
        ...
        match a {
            "--force"            => force = true,
            "--operator-name"    => { operator_name = Some(req(args, &mut i, a)?); }
            "--provider"         => { provider      = Some(req(args, &mut i, a)?); }
            "--provider-id"      => { provider_id   = Some(req(args, &mut i, a)?); }
            "--budget-usd"       => {
                budget_usd = req(args, &mut i, a)?.parse().map_err(|_| {
                    CliError::Usage(
                        "--budget-usd must be a non-negative integer".into(),
                    )
                })?;
            }
            "--max-concurrency"  => {
                ...
                if max_concurrency == 0 {
                    return Err(CliError::Usage(
                        "--max-concurrency must be > 0".into(),
                    ));
                }
            }
            "--plan-template"    => { plan_template   = req(args, &mut i, a)?; }
            "--initiative-name"  => { initiative_name = Some(req(args, &mut i, a)?); }
            "--skip-phase"       => { skip_phases.push(req(args, &mut i, a)?); }
            "--only-phase"       => { only_phase = Some(req(args, &mut i, a)?); }
    ...
    let new_fp = params_fingerprint(
        &operator_name, &provider, &provider_id, budget_usd, max_concurrency,
    );
    ...
    if want_phase(Phase::PolicyAuthoring) {
        if state.is_done(Phase::PolicyAuthoring) && !force {
            ...
            println!("[2/8] policy_authoring  — wrote {} ({} bytes)",
                     policy_path.display(), body.len());
    ...
    if want_phase(Phase::ProviderCreds) {
        ...
        println!("       raxis credential add --name {provider}-api-key --file ./{provider}-key.txt");
    }
    ...
    if want_phase(Phase::CredentialProxy) {
        println!("[5/8] credential_proxy  — DEFERRED (V3): edit plan.toml [[tasks.credentials]] entries");
    }
    ...
    if want_phase(Phase::PlanTemplate) {
        if state.is_done(Phase::PlanTemplate) && !force {
            ...
            let mut sub_args: Vec<String> = vec![
                "--template".into(),  plan_template.clone(),
                "--output".into(),    plan_path.display().to_string(),
            ];
    ...
    println!("  1. raxis cert mint --operator-name {operator_name:?} --output operator.cert.toml");
    println!("  2. raxis genesis --operator-cert ./operator.cert.toml --data-dir {}", data_dir.display());
...
fn req(args: &[String], i: &mut usize, flag: &str) -> Result<String, CliError> {
    let v = args.get(*i + 1).ok_or_else(|| CliError::Usage(format!(
...
fn params_fingerprint(
    ...
    h.update(op.as_bytes());          h.update(b"|");
    h.update(provider.as_bytes());    h.update(b"|");
    h.update(provider_id.as_bytes()); h.update(b"|");
    h.update(budget_usd.to_string().as_bytes());  h.update(b"|");
...
mod tests {
    ...
    fn render_policy_substitutes_all_placeholders() {
        let body = render_starter_policy(
            "alice", "anthropic", "anthropic-default", 25, 4,
        );
        ...
        let parsed: toml::Value = toml::from_str(&body)
            .expect("starter policy must be valid TOML");
