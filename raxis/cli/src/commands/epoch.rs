...
pub fn run_advance(flags: &GlobalFlags, args: &[String]) -> Result<(), CliError> {
            ...
            "--policy" => {
                ...
                policy_path =
                    Some(PathBuf::from(args.get(i).ok_or_else(|| {
                        CliError::Usage("--policy requires a path".to_owned())
                    })?));
            }
            "--sig" => {
                i += 1;
                sig_path =
                    Some(PathBuf::from(args.get(i).ok_or_else(|| {
                        CliError::Usage("--sig requires a path".to_owned())
                    })?));
            }
            other => {
                return Err(CliError::Usage(format!(
                    "unknown epoch advance flag: {other:?}"
                )))
            }
        }
        i += 1;
    ...
    handle_response(resp, |ok| {
        ...
        println!(
            "  new_epoch_id:               {}",
            ok["new_epoch_id"].as_u64().unwrap_or(0)
        );
        println!(
            "  policy_sha256:              {}",
            ok["policy_sha256"].as_str().unwrap_or("?")
        );
        println!(
            "  signed_by_authority:        {}",
            ok["signed_by_authority"].as_str().unwrap_or("?")
        );
        println!(
            "  n_delegations_marked_stale: {}",
            ok["n_delegations_marked_stale"].as_u64().unwrap_or(0)
        );
        println!(
            "  n_sessions_invalidated:     {}",
            ok["n_sessions_invalidated"].as_u64().unwrap_or(0)
        );
        println!(
            "  advanced_at:                {}",
            ok["advanced_at"].as_i64().unwrap_or(0)
        );
    })
}
