...
- `pub fn compute_admission_cost(touched_paths: &[PathBuf], intent_kind: IntentKind, policy: &PolicyBundle) -> Result<u64, BudgetError>`
  - For `IntentKind::IntegrationMerge`, returns `Ok(0)` before consulting policy. Integration merge is kernel-owned mechanical closeout, not delegated agent work, and must not consume lane cost or concurrency budget.
  - For every other priced intent kind, calls `policy.base_cost_for_intent_kind(intent_kind)`. If `None` (intent kind absent from cost table) → `Err(BudgetError::UnknownIntentKindCost { intent_kind })`. **No fallback cost is applied.** Unknown non-merge intent kinds are rejected at admission, not silently priced at a default. Parallel to `StrictDefault` in `policy_lookup::required_claims`: new surface area fails closed until policy acknowledges it.
