...
**Operator runbook implications.** Operators should monitor `raxis-cli status` (the v1 task-state dashboard) for tasks that have been in `Running` or `GatesPending` for longer than the deployment's expected per-task wall-clock budget. There is no audit event the operator can subscribe to that fires automatically when a task exceeds a wall-clock threshold; the operator's own monitoring layer (or a periodic CI cron that calls `raxis-cli status --json` and alerts on long-running tasks) is the v1 substitute. The kernel does emit `AuditEventKind::TaskTransitioned` on every state change, so a downstream log pipeline can compute "time since last transition" for each task and alert on outliers.
