...
| INV-CERT-01 | Every `[[operators.entries]]` block in any policy bundle the kernel will accept carries a self-signed `[operators.entries.cert]` sub-table — there is no legacy bare-pubkey path. Enforced at the loader (serde rejects `missing field "cert"`), at the canonical genesis emitter (cert sub-table is unconditional), at the store (`operator_certificates::repopulate` produces one row per entry), and at `raxis doctor` (an empty `operator_certificates` table is `[FAIL]`). | Without this, the audit chain cannot identify *who* approved a plan, expired keys never auto-fail-closed, and ambient operator authority defeats the four-zone least-privilege model. |
| INV-CERT-02 | Every accepted operator cert has been verified to be self-signed by the Ed25519 key whose public hex equals `cert.pubkey_hex`. **Unbypassable:** there is no `--force-misconfig` flag that relaxes this check at any layer (loader, CLI install, genesis embed). | Without this, an attacker with policy-file write access could mint arbitrary `permitted_ops` for any victim pubkey; the cryptographic anchor is what keeps the trust root unforgeable even when structural cert invariants are intentionally relaxed. |
| INV-CERT-03 | No CLI command writes operator private-key bytes to `<data_dir>` or any other persistent location. Private keys are read into process memory only for in-process `sign_cert` / `sign_policy` calls, then dropped. The kernel never sees the operator private key on any path. | Without this, a `<data_dir>` snapshot leak (mis-tuned backups, stolen disk, cloned VM) lets the attacker mint indistinguishable policy bundles; bounding the private key to the operator's own machine keeps the blast radius of a kernel-host compromise to "read-only forensic data." |
| INV-CERT-04 | When `raxis cert install --replace-for <fp> --new-cert <path>` rotates a cert, the new cert's `pubkey_hex` MUST equal the old cert's `pubkey_hex`. Audited via `OperatorCertInstalled.previous_fingerprint = Some(<old fp>)`. | Without this, an attacker (or careless operator) could silently swap one operator for another while the audit chain reads "rotation, not new operator," obscuring the change of authority; pinning pubkey continuity makes the audit chain's rotation walk unambiguous. |
| INV-CERT-05 | Every state transition involving an operator cert produces an audit event — install, rotation (with `previous_fingerprint`), structural bypass, expiry-window crossing, expired-op denial, emergency use. Per-event granularity, not per-policy-load rollups. | Without this, an investigator cannot reconstruct who held authority at any historical moment; per-event granularity is required to answer "did this specific approval execute under a then-active cert?" rather than just "was this cert installed at any point in the chain?". |
...
- Non-merge intent submitted with `IntentKind` not in policy cost table → `compute_admission_cost` returns `BudgetError::UnknownIntentKindCost`; handler returns `Ok(IntentResponse::Rejected { reason: FAIL_POLICY_VIOLATION })` directly (not through `map_error`); no task row binding `UPDATE`, no `lane_budget_reservations` insert, no `transition_task`. `IntegrationMerge` is the explicit exception: it is kernel-owned mechanical closeout and returns zero admission units before consulting policy. Verify: the task row (if it existed) is unchanged; no new reservation for that `task_id`.
...
Normative documentation ([`README.md`](../../../README.md), `specs/`, `fixtures/`) lives alongside the workspace in this repository; it is not part of the Rust dependency graph.
...
- `src/intent.rs` — Intent packet types: `IntentRequest`, `IntentResponse`, `BudgetSnapshot`, `PlannerErrorTemplate`. `IntentResponse` is the typed reply the kernel returns for every IPC `IntentRequest`; the Rust enum has exactly two variants and the canonical field sets below are the **normative** structural shape. Other sites in the spec that write `IntentResponse::Rejected { reason: X }` or `IntentResponse::Accepted { task_id: T, … }` are using an abbreviated illustration form that elides fields not pertinent to that example — they do not narrow the wire contract.

  ```rust
  enum IntentResponse {
      Accepted {
          task_id: TaskId,                              // bound task; not serialised on the wire (see `peripherals.md` §3.1) — wire correlation is via envelope `sequence_number`
          task_state: TaskState,                        // current state of the bound task after this intent applied (`Running`, `GatesPending`, `Completed`, …)
          remaining_budget: BudgetSnapshot,             // per-lane budget after `consume_budget`; inner shape defined by the `BudgetSnapshot` struct in this same crate (units pinned by `kernel-core.md` §3.5 / `kernel-store.md` §2.5.7 admission-units model)
          warn_delegation_stale: bool,                  // true iff `evaluate_claims` took the `SufficientStale` grace use; planner must renew before next gated action
      },
      Rejected {
          reason: PlannerErrorCode,                     // coarse code per §1.3 INV-08; full enum normatively listed in `planner-api.md`
          error_detail: Option<PlannerErrorTemplate>,   // INV-08: `Some(template)` only when `reason == FAIL_POLICY_VIOLATION`; `None` for every other code (enforced at the type level by the dispatcher — see `kernel-core.md` `dispatcher::map_error`)
          task_state: TaskState,                        // current task state observed at rejection time — informs planner retry strategy (e.g. wait for DAG predecessors, gate clearance, or recovery)
      },
  }

  struct BudgetSnapshot {
      admission_units: u64,                         // remaining admission units on the task's lane after `consume_budget` ran for this intent (per `kernel-core.md` §3.5 `compute_admission_cost` semantics: NOT a token count, USD amount, or wall-clock estimate — admission units are the kernel's coarse lane-saturation unit; serialised on the wire as the JSON object `{ "admission_units": <u64> }` per `peripherals.md` §3.1)
  }

  enum PlannerErrorTemplate { /* fixed, version-controlled generic-template strings — defined in `raxis-types/src/error.rs` per `peripherals.md` §3.1 INV-08 rule */ }
  ```
...
**What it contains:**
- `src/signing.rs` — Sign and verify policy artifacts. Wraps a signing backend (v1: `ring` or `ed25519-dalek`). The authority signing key and quality signing key have different key types registered in the key registry — this is enforced here, not by convention. **In v1, only the authority key is exercised by this module** (it signs `ApprovalProof` records and verifies the policy artifact); the quality key type exists in the registry for forward compatibility with v2 witness-record signing but no v1 code path calls `quality_key.sign(…)` — see [`kernel-store.md`](kernel-store.md) §2.5.4 `quality_keypair` row and [`kernel-core.md`](kernel-core.md) `KeyRegistry` note. **Three-way key-domain split (read this once, refer back as needed):** `ApprovalProof` receipts are signed by the **kernel's `authority_keypair`** (the kernel's own runtime keypair, also used to verify the policy artifact); **plan artifacts** (`plan.toml` + `plan.sig`) are signed by the **operator's** Ed25519 private key from `[[operators.entries]]` (verified by the kernel against `policy.operator_entry(plan.sig.signed_by).public_key`); **approval tokens** are also signed by the **operator's** private key (see "Note on approval tokens" below) and verified the same way. The kernel's `authority_keypair` never signs plan artifacts or approval tokens; the operator's private keys never sign `ApprovalProof` receipts. The four-key custody model in [`kernel-store.md`](kernel-store.md) §2.5.4 (Table: `authority_keypair`, `operator_key`, `quality_keypair`, `verifier_token_key`) is the canonical reference; §2.5.3 "Plan Artifact Signing Contract" pins the plan-signing path specifically. **Note on approval tokens:** `ApprovalToken` signing is performed by the operator CLI using the operator's own private key (not the kernel's authority keypair) and is not part of this module's responsibility. Verification of approval tokens uses `policy.operator_entry(token.issued_by).public_key` loaded from the policy artifact — see `authority/approval.rs`.
...
- `src/keyring.rs` — Key registry: loads key entries from the policy store, provides a typed lookup (`AuthorityKey`, `QualityKey`, `VerifierTokenKey`). The authority key and quality key are different entries with different capability grants — the compiler enforces this by making them separate types. **`QualityKey` is loaded but unused in v1**: the registry exposes the type so v2 witness-record signing can adopt it without a key-loading code change, but no v1 caller asks the registry for `QualityKey`. A v1 module that fetches `QualityKey` from the registry should fail code review — the unused-key invariant is enforced at the call-site, not by absence of the type.
...
- `src/loader.rs` — Load and verify signed policy artifacts. Load sequence:
  ...
  6. Check `raw.meta.epoch > store.read_current_epoch()` where `store.read_current_epoch()` is implemented as `SELECT COALESCE(MAX(epoch_id), 0) FROM policy_epoch_history` against Table 19 (see [`kernel-store.md`](kernel-store.md) §2.5.1) and returns `0` if the store has never recorded an epoch (genesis boot). The comparison is strictly greater than — loading the same epoch twice is rejected as `PolicyError::EpochReplay`. This rule is identical at boot and at `advance_epoch`; the only difference at genesis is that `read_current_epoch()` returns `0`, which any valid first artifact (epoch ≥ 1) satisfies. Genesis itself inserts the first row into `policy_epoch_history` (with `triggered_by_operator = "genesis"`) under the same transaction that finalises the schema, so by the time the kernel begins servicing any IPC, MAX(epoch_id) is non-zero and matches the in-memory `PolicyBundle.epoch_id`.
...
- `src/bundle.rs` — **[NEW]** `PolicyBundle` struct and accessor methods. Constructed by `loader::load_and_verify`. This is the in-memory, fully validated representation of a loaded policy artifact. Wrapped in `ArcSwap<PolicyBundle>` by `policy_manager.rs`.
  - `pub fn base_cost_for_intent_kind(&self, kind: IntentKind) -> Option<u64>` — returns `None` if kind absent from `budget.base_cost_per_intent_kind`; callers treat `None` as `UnknownIntentKindCost` for priced non-merge intents. `IntegrationMerge` is budget-neutral and bypasses this lookup.
...
# Absent priced non-merge variant → UnknownIntentKindCost at admission (FAIL_POLICY_VIOLATION).
...
- `src/audit_record.rs` — `render_genesis_audit_record(GenesisAuditInputs) -> String`. Pure function — caller injects the 64 CSPRNG bytes (sourced from `raxis_crypto::token::try_random_array`), the `event_id` UUID, and the wall-clock timestamp. The emitter validates the nonce length is exactly 64 bytes (= 512 bits, with 256-bit headroom over the spec floor in [`kernel-store.md`](kernel-store.md) §2.5.5 `audit-genesis-nonce`).
...
- `raxis-cli genesis …`, epoch advance, break-glass flows as specified in [`cli-ceremony.md`](cli-ceremony.md)
...
RAXIS is local-first. The CLI binary runs on the same host as the kernel and has Unix-level access to `<data_dir>/`. Routing read queries through the kernel's IPC socket would couple every operator dashboard to a new kernel handler + IPC schema bump + audit emit — exactly the brittle, ever-growing kernel surface v1 is designed to avoid. The read-only CLI therefore opens `kernel.db` directly with `SQLITE_OPEN_READ_ONLY` (the kernel's mandatory `PRAGMA journal_mode = WAL` setup, see [`kernel-store.md`](kernel-store.md) §2.5.1, guarantees readers and writers never block each other) and parses the audit JSONL directly. Schema coupling is mitigated by routing every read through `raxis-store::views::*` — a typed query catalog shared with the kernel — and pinned via a compile-time `EXPECTED_SCHEMA_VERSION` check. INV-08 (path-list confidentiality) is preserved at the file-system boundary by a `Redactable<T>` redaction layer that requires explicit `--reveal-paths` AND emits an auditable `PathReadAccessed` event before returning sensitive fields. See [`cli-readonly.md`](cli-readonly.md) §5.1 for the full architectural rationale and §5.7 for the security model.
