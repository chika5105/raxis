...
| `AuditSink` | Local JSONL / PostgreSQL / S3 + object-lock / Sigstore-Rekor transparency log | `R-7` chain math stays in the kernel; the sink only persists already-sealed bytes. |
...
Subsystems that *enforce* an `R-*` invariant — the intent admission pipeline, the policy parser, the escalation FSM, the hash chain, the wire framing, session token model, policy-epoch-advance transactional shape, plan/policy hierarchy narrowing, worst-case-budget reservation, and the touched-paths derivation — stay concrete because abstracting them would let an impl claim RAXIS conformance while not actually enforcing the paradigm.
