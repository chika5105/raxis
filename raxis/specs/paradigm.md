...
- Implementation language (Rust, Go, Zig, C++, formally verified Coq, etc., are all viable)
...
Two adjacent concepts deserve explicit disambiguation because they share surface-level overlap with RAXIS:

- **An IAM system for AI agents.** IAM (Identity and Access Management) answers "is this principal allowed to perform this action on this resource?" — authenticate, evaluate policy, allow/deny, log. RAXIS shares the authentication (R-9), policy evaluation (R-3, R-4, R-5), and audit (R-7) properties. But IAM is a gatekeeper at the API boundary: once the principal passes the gate, IAM is done and the principal executes freely within its permissions. RAXIS differs in five ways IAM does not address: (1) domain separation (R-1) — IAM does not control where the principal executes; (2) continuous I/O mediation (R-2) — IAM grants access at the boundary, not on every I/O path; (3) quantitative capability bounds (R-5) — IAM has rate limits but does not require every capability to carry explicit numerical ceilings; (4) adversarial opacity (R-10) — IAM returns detailed denial reasons, RAXIS actively prevents the principal from learning which rule fired because the principal is an adaptive adversary; (5) coordination mediation (R-11) — IAM does not govern inter-principal communication. An IAM-gated agent framework is a useful building block but does not satisfy R-1, R-2, R-10, or R-11.

- **A reference monitor (Anderson, 1972).** A reference monitor provides three properties: complete mediation (every access is checked), tamper-proofness (the monitor cannot be modified by the subject), and verifiability (small enough to be analyzed). RAXIS satisfies all three — R-2 is complete mediation, R-1 is tamper-proofness, R-3 is verifiable policy. But a reference monitor is a *mechanism*; RAXIS is a *paradigm* that includes a reference monitor and adds five properties a classical reference monitor does not address: (1) quantitative bounds (R-5) — reference monitors make binary allow/deny decisions, RAXIS requires numerical ceilings on every grant; (2) adversarial opacity (R-10) — reference monitors do not care whether the subject learns the denial reason because the subject is a deterministic process, RAXIS prevents it because the subject is an LLM that will probe the policy boundary; (3) dynamic escalation (R-12) — reference monitors are static (denied is denied), RAXIS provides a structured out-of-band escalation path to a human through a channel intelligence cannot influence; (4) reproducible audit (R-8) — reference monitors log decisions, RAXIS requires that every decision be replayable from the audit chain by an independent verifier; (5) coordination mediation (R-11) — reference monitors mediate subject-to-object access, RAXIS also mediates subject-to-subject communication. The deeper difference is the principal model: a reference monitor assumes the subject is a deterministic process whose behavior is bounded by its instruction set; RAXIS assumes the subject is a non-deterministic intelligence whose behavior is unbounded, potentially prompt-injected, and fundamentally unpredictable. **A reference monitor enforces static access control on deterministic subjects. RAXIS enforces bounded, auditable, escalatable authority over non-deterministic intelligence.**
...
| **R-6 Fail-Closed Default** | `INV-01` (planner cannot act without session token); `INV-CAPACITY-04` (audit write impossible halts kernel); `INV-RETRY-FROM-COMPLETED-REVIEW-REJECTED-01` (retry from a clean `Completed` activation is REJECTED; admission requires `review_reject_count > 0`); fail-closed admission throughout `kernel/src/handlers/` |
...
### 6.1 Pluggable trait boundaries (reference-implementation property)

The reference implementation exposes six trait seams at which alternative implementations can substitute the V2 default without weakening any `R-*` invariant. These seams are **reference-implementation choices**, not paradigm requirements — a different RAXIS could pick a different set of seams or none at all, as long as the resulting system still satisfies the twelve `R-*` invariants. The seams in this implementation are listed in the table below; the canonical spec is [`v2/extensibility-traits.md`](v2/extensibility-traits.md).

| Trait | What can be substituted | Why substitution does NOT weaken any `R-*` |
|---|---|---|
| `DomainAdapter` | SE / trading / healthcare / robotics intent kinds and merge semantics | Domain-agnostic per §5; SE-specific operations are not paradigm-level. |
| `IsolationBackend` | Firecracker / Apple-VZ / Intel-TDX / AMD-SEV-SNP / future enclaves | `R-1` permits "at least equivalent to a hardware-virtualised microVM"; conformance kit verifies equivalence. |
| `CredentialBackend` | File / HashiCorp Vault / AWS Secrets Manager / Azure Key Vault / PKCS#11 HSM | `R-2` requires mediation, not a specific store; the gateway-only-reads property is preserved. |
| `AuditSink` | Local JSONL / PostgreSQL / S3 with object-lock / Sigstore-Rekor transparency log | `R-7` chain math stays in the kernel; the sink only persists already-sealed bytes. |
| `OperatorTransport` | UDS / mTLS gRPC / WebSocket-over-HTTPS / serial-USB relay | `R-9` and `R-12` require unforgeable + human-authenticated; the Ed25519 challenge-response ceremony is unchanged across transports. |
| `InferenceRouter` | HTTPS gateway / local vLLM / on-prem TGI / in-cluster gRPC inference | `R-2` mediation is satisfied by any router that strips planner authority over destination + meters tokens; specific providers are deployment choices. |

Subsystems that *enforce* an `R-*` invariant — the intent admission pipeline, the policy parser, the escalation FSM, the hash chain, the wire framing, session token model, policy-epoch-advance transactional shape, plan/policy hierarchy narrowing, worst-case-budget reservation, and the touched-paths derivation — stay concrete because abstracting them would let an implementation claim RAXIS conformance while not actually enforcing the paradigm.
