...
> - [`policy-plan-authority.md`](policy-plan-authority.md) (`[plan_bundle_limits]` policy section, FAIL_PLAN_BUNDLE_* codes)
...
> - [`key-revocation.md`](key-revocation.md) (operator key lookup at admission)
> - [`v2-deep-spec.md`](v2-deep-spec.md) Step 17 (`approve_plan` shift-left validation)
...
- The semantic content of `plan.toml` itself (covered in
  [`policy-plan-authority.md`](policy-plan-authority.md), [`planner-harness.md`](planner-harness.md), [`verifier-processes.md`](verifier-processes.md),
  [`custom-tools.md`](custom-tools.md), etc.). Plan Bundle Sealing is the **transport and
  storage** layer; what fields the plan declares is orthogonal.
- Operator key custody and revocation ([`key-revocation.md`](key-revocation.md)).
...
(`v1/kernel-store.md` §2.5.5, [`key-revocation.md`](key-revocation.md) §3). This prevents
...
or creating an initiative; see [`operator-ergonomics.md §12`](operator-ergonomics.md) for the
...
> ([`operator-ergonomics.md §5`](operator-ergonomics.md)) so that policy-resolved defaults are
...
> ([`policy-plan-authority.md §5 step 0e`](policy-plan-authority.md)).
...
### 8.1 Admission sequence (extends [`policy-plan-authority.md`](policy-plan-authority.md) §5)
...
The kernel runs a periodic sweep (alongside the §10 / [`kernel-lifecycle.md`](kernel-lifecycle.md)
maintenance loop):
...
[`policy-plan-authority.md`](policy-plan-authority.md) §3 (with this spec as the authoritative
...
| `FAIL_PLAN_REQUIRES_PREPARE { missing_fields }` | Kernel admission step 0e | The plan omits at least one defaultable field whose policy default is set; the operator did not run `raxis-cli plan prepare` first. Canonical home: [`operator-ergonomics.md §20`](operator-ergonomics.md). Listed here for cross-reference because it fires on `submit plan` admission alongside the other Plan Bundle Sealing checks. |
...
- Forensic post-mortems on a compromised key ([`key-revocation.md`](key-revocation.md))
...
| [`key-revocation.md`](key-revocation.md) | Operator key lookup at admission is unchanged; `signed_by` fingerprint resolves through the same `policy.operators` path. The set of FAIL codes a revoked key produces is unchanged. The §3.5 freshness window is **complementary** to key revocation: a revoked key still rejects admission via `FAIL_KEY_*`; a fresh key with a stale bundle still rejects admission via `FAIL_PLAN_BUNDLE_EXPIRED`. The two checks are independent and ordered (key check first per §8.1 step 10, freshness/replay second per §8.1 step 10a–10b). |
| [`kernel-lifecycle.md`](kernel-lifecycle.md) | Add the §8.4 nonce-table sweep to the kernel's periodic-maintenance loop (cadence aligned with the existing audit-retention sweep; default once per hour). |
| [`kernel-mechanics-prompt.md`](kernel-mechanics-prompt.md) | KSB rendering reads from `plan_bundle_artifacts` instead of `<data_dir>/plans/<initiative_id>/plan.toml`. The KSB content itself is unchanged; only the byte source moves into the SQLite store. |
| [`custom-tools.md`](custom-tools.md) | Already updated by D1: `command_sha256` removed entirely. Custom-tool scripts live in the operator's VM image; no host-side bundling of script bytes. |
...
- [ ] Signed by revoked key → `FAIL_KEY_COMPROMISED` (per [`key-revocation.md`](key-revocation.md)).
