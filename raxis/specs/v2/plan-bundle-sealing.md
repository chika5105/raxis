...
| **Kernel admission (§8.1)** | Pending | New `OperatorRequest::CreateInitiative` wire envelope; §8.1 step ordering (envelope decode → key lookup → `BEGIN IMMEDIATE` → freshness/replay/policy/admission). |
