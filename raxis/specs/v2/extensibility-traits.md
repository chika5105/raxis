...
- `cli/src/commands/audit.rs` — adds `raxis verify-chain` (kernel-linked convenience wrapper using `raxis_audit_verify::verify`; NOT the R-7 artefact — that is `raxis-audit-verify` in the leaf crate); adds `raxis audit export-state-for-verifier` (writes JSON consumable by `raxis-audit-verify --state-export`); adds `--acknowledge-critical` flag on `raxis verify-chain` per `audit-paired-writes.md §6.2` for the boot-block override (signed with the standalone binary's verdict hash)
