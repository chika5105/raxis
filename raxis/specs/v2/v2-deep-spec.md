...
| **Extensibility traits** — six pluggable trait boundaries (`DomainAdapter`, `IsolationBackend`, `CredentialBackend`, `AuditSink`, `OperatorTransport`, `InferenceRouter`); the rule that decides what gets a trait; per-trait file enumeration; conformance contracts; phased migration plan | [`extensibility-traits.md`](extensibility-traits.md) | V2 Specified |
