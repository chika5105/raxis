...
**Implementation reference (V2 — installer CLI shipped).** The reference newsyslog snippet ships under [`raxis/installer/newsyslog/raxis.conf`](../../installer/newsyslog/raxis.conf), the macOS LaunchDaemon under [`raxis/installer/launchd/com.raxis.kernel.plist`](../../installer/launchd/com.raxis.kernel.plist), and the Linux systemd unit under [`raxis/installer/systemd/raxis-kernel.service`](../../installer/systemd/raxis-kernel.service). All three files are self-documenting at the top with install / uninstall commands. The convenience CLI surface ships as `raxis kernel install [--system] [--binary <path>] [--force]` (per `cli/src/commands/kernel.rs`), which templates the unit file with the operator's binary path + `--data-dir`, places it at the platform-canonical location (`~/.config/systemd/user/raxis-kernel.service` for Linux user, `/etc/systemd/system/raxis-kernel.service` for Linux system, `~/Library/LaunchAgents/com.raxis.kernel.plist` for macOS user, `/Library/LaunchDaemons/com.raxis.kernel.plist` for macOS system), and prints the next-step `systemctl --user enable --now raxis-kernel` / `launchctl bootstrap` command. `raxis kernel uninstall [--system]` removes the unit file and prints the corresponding cleanup commands. The full `raxis kernel start --daemon` / `stop` / `status` / `restart` lifecycle (with `sd_notify` integration, single-instance enforcement, and crash-loop detection per §8) is deferred to a follow-up sprint; until then operators drive start/stop through the platform supervisor (`systemctl --user start raxis-kernel`, `launchctl kickstart`, etc.) directly after running `raxis kernel install`. `guides/SETUP.md` step 7 includes a pointer to both the CLI and this folder.
