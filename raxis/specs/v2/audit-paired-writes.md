...
Not every audit event participates in the paired protocol. The full classification is below. The classification is **load-bearing** — implementers must never invent a third class — and is enforced by the spec-graph lint ([`v2-deep-spec.md §Spec-Graph Lint`](v2-deep-spec.md)):
...
**Out-of-scope events** (single, not paired): `AuditEventKind::VmImageResolved` (V2.5 BYO-image mechanical witness — [`canonical-images.md §3`](canonical-images.md); emitted once per Executor activation that resolves a `[[vm_images]]` alias, BEFORE the VM-spawn step; success-path observability with no SQLite row mutation, so single-class), `AuditEventKind::SecurityViolationDetected` (kernel-internal trust-boundary check fired from `canonical_images_preflight.rs` and from `handle_activate_sub_task` on operator-image digest mismatch; observability for an irrecoverable security event with no follow-up state row), `AuditEventKind::InferenceRequested`, `AuditEventKind::InferenceCompleted`, all `FAIL_*` Phase-A rejections, `AuditEventKind::Heartbeat`, `AuditEventKind::KSBSnapshot`, `AuditEventKind::RaxisDoctorChecksRan`, `AuditEventKind::MaintenanceTickStarted/Completed/Skipped`, `AuditEventKind::GenesisRecord`, `AuditEventKind::AuditSegmentRotated`, `AuditEventKind::AuditSchemaMigration`, `AuditEventKind::IsolationSubstrateSelected` (boot-time substrate identity record, no state row), `AuditEventKind::IsolationFallbackBypass` (warning at boot, no state row), `AuditEventKind::NotificationDelivered`, `AuditEventKind::NotificationDeliveryFailed`, `AuditEventKind::NotificationTestSent` (the *test sent* is paired via `NotificationDispatchClaimed`; the *outcome* is single), `AuditEventKind::CredentialAccessed`, `AuditEventKind::CredentialRotated` (resolution / rotation observability emitted by `CredentialBackend`; no SQLite row mutates), `AuditEventKind::TransparentProxyAdmitted`, `AuditEventKind::TransparentProxyDenied` (admission outcome observability per [`vm-network-isolation.md §3.2`](vm-network-isolation.md)), `AuditEventKind::DefaultProviderEgressApplied`, `AuditEventKind::SessionEgressStallDetected` (reviewer/default-egress observability; no state row mutates), `AuditEventKind::CredentialProxyStarted`, `AuditEventKind::CredentialProxyStopped` (proxy-lifecycle observability per [`credential-proxy.md §5`](credential-proxy.md); the proxy registry row state-change goes through its own paired event), `AuditEventKind::DatabaseQueryExecuted`, `AuditEventKind::HttpProxyRequestExecuted` (per-request observability emitted by the credential proxies; no SQLite row mutates per query).

### §4.2 Spec-graph lint enforcement

`xtask spec-graph` (per [`v2-deep-spec.md`](v2-deep-spec.md)) gains a new check: every variant of `AuditEventKind` MUST appear in either the paired-class list above OR in the explicit single-class list. A variant in neither list is a compile-error from the lint. New audit event kinds added in future PRs MUST update one of the two lists.
...
| `crates/audit/src/sink.rs` | Extend `AuditSink` trait per [`extensibility-traits.md §5`](extensibility-traits.md) with `emit_pending`, `emit_confirmed_for`, `emit_rolled_back_for`, `emit_recovered_confirmed`, `emit_recovered_rollback` |
...
| [`raxis/specs/v1/kernel-store.md`](../v1/kernel-store.md) | §2.5.2 AuditSink ordering rewritten as the V2.1 paired ordering; cross-reference this spec; add `last_committing_event_seq` column to schema docs |
| [`raxis/specs/v1/kernel-core.md`](../v1/kernel-core.md) | Intent admission pipeline — Phase B insertion; `recovery::reconcile` → `reconcile_advisory`; cross-reference this spec |
| [`raxis/specs/v1/cli-readonly.md`](../v1/cli-readonly.md) | New §5.5.19 `raxis audit export-state-for-verifier` (kernel-linked exporter for the standalone binary's `--state-export` mode) |
| [`raxis/specs/v2/extensibility-traits.md`](extensibility-traits.md) | §5 (`AuditSink`) extended with paired-write methods; §5.3/§5.4 file lists realigned to point at `crates/audit-verify/` (leaf crate) for the verifier algorithm |
| [`raxis/specs/v2/policy-plan-authority.md`](policy-plan-authority.md) | New `FAIL_AUDIT_*` failure codes |
| [`raxis/specs/invariants.md`](../invariants.md) | New `INV-AUDIT-PAIRED-01..07` |
| [`raxis/specs/v2/v2-deep-spec.md`](v2-deep-spec.md) | Register this spec in Related Specifications; spec-graph lint extension |
...
See `specs/invariants.md §INV-FAILURE-REASON-MANDATORY-01` for the canonical templates and the regression-alarm rationale.
