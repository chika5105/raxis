...
Default buckets (ms) are `[1, 5, 10, 25, 50, 100, 250, 500, 1000, 2500, 5000, 10000]`. Operators MAY override via `[observability.metrics.histogram_buckets]`. Buckets are global (one set covers every histogram); per-metric overrides are out of scope for V3 to keep validation simple.
