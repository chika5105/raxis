...
iter44 introduces a small set of perf-metric outliers whose latency profile spans several decades wider than the global default — kernel respawn (10 ms cold-restart through 5 minute crash-loop back-off) is the canonical example. Those metrics use `ObservabilityHub::record_histogram_with_buckets` to declare per-call buckets at the emit site; the per-metric buckets are listed in the relevant `§8` row above. The hub-wide `[observability.metrics.histogram_buckets]` setting remains the single source of truth for every other histogram, so existing dashboards keep rendering against the same cumulative buckets they always have.
