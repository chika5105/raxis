...
| You are... | Start here |
|---|---|
| **An operator / evaluator** who wants to run RAXIS now | [`guides/getting-started/`](guides/getting-started/) |
| **A source builder** compiling RAXIS locally | [`guides/SETUP.md`](guides/SETUP.md) |
| **A maintainer** changing release, Homebrew, notarization, or bottles | [`release/README.md`](release/README.md) |

### 1. Install with Homebrew

For end users, Homebrew is the primary install path:
...
Then run the first initiative:
[`guides/getting-started/02-first-initiative.md`](guides/getting-started/02-first-initiative.md).

The Homebrew bottle ships the CLI, kernel, gateway, dashboard bundle,
canonical VM images, and guest kernel. You do not need Rust, Node.js,
Docker, or image baking for the operator quick start.

### 2. Build and Install from Source

RAXIS is written in Rust. Source builds are for developers and release
maintainers. For a fresh source build, use the one-shot setup wrapper:
...
### 3. Set Up Your Operator Identity
...
### 5. Planner worktree and session bootstrap
...
| **Executor custom tools** | Operator-defined tools live in `plan.toml` profiles as `[[profiles.<name>.custom_tool]]`, not in `policy.toml [[tools]]`. The kernel validates the signed plan, resolves profile inheritance, rejects task-level tools, rejects Reviewer/Orchestrator tool surfaces, and stamps only the effective Executor bundle into that task's session. Each tool is an absolute wrapper command that receives JSON stdin, returns a `ToolOutput` JSON envelope, and carries an explicit schema plus `timeout_seconds` capped by policy (default hard cap: 300s). This is the supported BYO/MCP path: wrap one existing script or MCP method per bounded custom tool, for example `unity_build_player` or `blender_export_asset`; do not expose a generic `mcp_call_anything` bridge. For required sequences, use one composite tool that emits receipts/artifacts plus a verifier that gates `CompleteTask`. |
