...
The proxy is allowlisted to forward `find` on `seeded_docs` and
`SELECT` on `seeded_rows`, so the helper below has been tested
end-to-end against the canonical proxy plumbing.
...
def materialize_mongo() -> int:
    # The proxy is a `mongodb://127.0.0.1:PORT/` URL with no
    # userinfo and no default-database segment — pass the database
    # name explicitly with `client["raxis_e2e_mongo"]`, do NOT call
    # `client.get_default_database()` (it would raise
    ...
    # forces standalone topology so the driver does not try to
    # speculate about replica-set / mongos shapes that the proxy
    # does not emulate.
...
* Your `path_allowlist` is **enforced at `task_complete` time
  against the full git diff** (`base..head`). Any file that
  appears in the commit but is NOT under `out/postgres/`,
  `out/mongo/`, or `out/manifest.json` will fail the path-scope
  check with `FailPathPolicyViolation`, the kernel will mark the
  task `Failed`, and you will burn a `crash_retry_count` slot.
