...
| `plan_primary.toml`                       | `realistic_plan_toml()` in `extended_e2e_support::plan_realistic`. The primary initiative (materializer + xfile-refactor + lint-defect/lint-runner/reviewer fan-out + repeated `workspace_merge` fan-in + allowlist + service/proxy/credential/egress/tooling evidence). |
