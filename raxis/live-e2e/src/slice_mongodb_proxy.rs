...
use raxis_credential_proxy_mongodb::{
    NoopAuditChannel, OwnedConsumer, ProxyConfig, MongodbProxy, Restrictions,
...
pub async fn run() -> Result<()> {
    ...
    // 1. hello → ok: 1.0
    drive_command(&mut sock, 1, "hello").await
        .context("hello")?;
    ...
    if snap.commands_audited != 4 {
        return Err(anyhow!(
            "expected commands_audited=4, got {}", snap.commands_audited,
        ));
    }
    if snap.commands_blocked != 1 {
        return Err(anyhow!(
            "expected commands_blocked=1, got {}", snap.commands_blocked,
        ));
    }
...
async fn drive_command(
    ...
    if first != "ok" {
        return Err(anyhow!(
            "{command} reply first field {:?} != 'ok'", first,
        ));
    }
...
async fn read_message(sock: &mut TcpStream) -> Result<Option<(MsgHeader, Vec<u8>)>> {
    ...
    if h.message_length < HEADER_LEN as i32 {
        return Err(anyhow!(
            "MongoDB header length {} smaller than HEADER_LEN", h.message_length,
        ));
    }
