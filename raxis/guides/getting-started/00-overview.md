...
| **Plans as signed bundles.** A plan is a TOML file whose canonical bytes are hashed and signed atomically by the CLI before the kernel sees them. Re-reading the plan from disk after admission is forbidden by `INV-INIT-06`. | `raxis submit plan <plan.toml>`                                                                                      |
